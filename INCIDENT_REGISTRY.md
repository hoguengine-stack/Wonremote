...
- Status: Source repairs and focused regression checks passed. Non-public 0.1.106 candidate CI passed and its Agent is installed on 82220F6D with original identity, fresh online health and a valid running secure broker. User-visible cross-PC checks remain open.
...
- Fix commit(s): Uncommitted focused source repair.
...
- Release proof: No public deployment is authorized or performed. CI `35248926285` completed from `1b87b4380033d89e0a0bbbdbb706148b4bc67354` with `publish=false`; downloaded Viewer/Agent installers and manifest passed trusted signature/hash verification. Agent 0.1.106 installed only on 82220F6D, preserved `123-45-67890:AGENT-82220F6D` / `82220F6D`, returned a fresh accepted heartbeat and runs the exact SYSTEM/Highest secure broker. Public latest remains v0.1.105.
...
- Remaining blocker: Install the downloaded 0.1.106 Viewer candidate on the other test PC, then verify Agent icon, Korean IME, lock-to-PIN-to-unlock live frames and input without a device-list refresh, exact tray restart sequence, and a large-file pause/resume/cancel/final-save cycle between two Windows PCs. Android remains out of scope.
