...
- Severity: High; lock-screen continuity and post-unlock control can be unavailable, while other failures make core input and transfer operations misleading or unusable.
- Affected: Agent taskbar/tray role UI and restart lifecycle; Viewer Hangul toggle; normal/secure desktop capture and session recovery; Viewer-to-Agent and Agent-to-Viewer file-transfer controls/finalization. Android is explicitly deferred.
- Status: Source repairs and focused regression checks passed. Public v0.1.107 was built and published from the exact main commit, and the production rollout policy is active for General at 100 percent. User-visible cross-PC checks remain open.
- User-visible symptom: Agent taskbar icon shows Viewer V; Hangul toggle appears combined with a modifier; lock/PIN frame stalls and post-unlock session may remain stale until device-list refresh; restarted Agent tray icon does not respond; transfer panel lacks pause/cancel/resume and independent close; 100% receipt is followed by a separate save phase.
- Minimal trigger: On this PC's public 0.1.105 Agent, enter the Windows secure desktop during an active Viewer session. The Agent logs `secure-desktop-required` but does not stop or restart that capture process; Viewer retains the prior frame until the session is ended and recreated. In Viewer, hold a tracked modifier then press Hangul, or receive a file and inspect the transfer controls/finalization labels.
- Root cause and contributors: The Agent changed its secure/default desktop state after the native transition event but left the capture process attached to the previous desktop. A device-list reconnect created a new session and fresh capture, hiding the missing same-session restart. Non-elevated task enumeration reports the protected task as not found even though a scheduler-owned session-0 POC process is present, so the initial absent-task diagnosis was invalid. The Hangul branch moved focus to local IME without releasing remote-owned Ctrl/Shift/Alt/Win state. Agent mode used the shared Viewer icon. Incoming transfer state had persisted resume data but no pause/cancel distinction or independent panel visibility, and displayed network receipt separately from an unexplained local-save phase. Current installed logs show tray interaction succeeding after child restart, so no unsupported tray root cause is asserted.
- Fix commit(s): `574c48eb90416c374de1e10181563f20676a9118`.
- Permanent guard: A real secure/default desktop-class transition stops only the current capture, ignores its late stale frames, restarts immediately with a fresh keyframe, and preserves the WebRTC/input/file session. Repeated same-class signals cannot loop, and a failed process-stop request leaves transition state retryable. Ordinary startup does not infer broker absence from a hidden task; elevated update success still requires the exact SYSTEM/Highest/running secure broker. Handler tests hold modifiers through Hangul. Packaging tests require distinct role icons and preserve tray ownership across child restart. Rendered transfer tests cover close/reopen, pause, ID-bound resume, cancel/discard, retry and 99%-until-durable-save semantics. Transfer lifecycle controls remain in the status modal rather than the session tools; after desktop auto-save, the completed row opens only the exact existing file inside the configured local download folder. Android remains excluded.
- Regression proof: Compared known working v0.1.88 commit `04cb8336253e5d102bdd9f58af69ae19b4f7e1a5` with affected current commit `76b9391363b0d89de149f2517b17eea4f36bb583`, then correlated the reported lock action with the installed log: the transition event appears with no capture exit/restart until session teardown; fresh-session capture then works. A scheduler-owned session-0 POC process is present despite non-elevated task enumeration reporting not found. The broad scoped run executed 350 cases: 349 passed immediately and the one corrected test assertion then passed; the final changed-boundary rerun passed 124/124. TypeScript, contract JSON parsing and diff checks passed. Incoming/outgoing transfer screenshots show contained controls without clipping. Earlier unchanged x86 Tauri icon/tray checks remain valid. The installed tray log independently records successful post-restart left click, menu, show and focus.
- Test-development correction: The first aggregate run passed 349 product cases and failed one new source assertion because it expected callback invocation syntax while production correctly passed the callback into the tested transition function. The assertion now checks the real callback wiring; no product code or acceptance condition was weakened.
- Release proof: CI `35264060553` completed all change, release, installer-update, startup, scheduled-task handoff, native scope, manifest and publication gates from exact main commit `574c48eb90416c374de1e10181563f20676a9118`. Public latest v0.1.107 is non-draft and non-prerelease. Independently downloaded Viewer/Agent installers and manifest matched release SHA-256 values and passed trusted manifest verification; both PE versions are 0.1.107. All four Firebase aliases point to the matching latest assets. The production rollout policy was atomically changed to 0.1.107, General, 100 percent and active with no selected-device restriction; the one existing per-device pause was preserved. No fresh device report followed during this verification window, so fleet installation is not claimed.
- Test-deployment gate correction: The first complete source gate correctly rejected the stale `active` status and a manual-update outcome that omitted the newly changed secure-broker health test. No artifact had been built. The contract now records source verification separately from pending deployment evidence and links the changed health test; the complete gate must pass before commit or CI dispatch.
- Candidate CI timeout correction: CI `35248072509` passed change and version gates but stopped before packaging because the real PowerShell health predicate exceeded Vitest's default five-second test timeout under the combined Windows release workload. The PowerShell child remains capped at ten seconds and all predicate/count assertions remain unchanged; only that integration test now has a 20-second outer allowance. No artifact or release was produced by the failed run.
- Evidence-hash correction: While moving completed source outcomes out of the active follow-up contract, an unverified expansion of short commit `92ca13b` was briefly entered. It was caught before commit or CI dispatch and replaced with direct `git rev-parse HEAD` output `92ca13b3bfc7b87698b6d191a4ba5cd4eb5555d7`. Full evidence hashes must be copied only from a repository query.
- Remaining blocker: On updated physical PCs, verify Agent icon, Korean IME, lock-to-PIN-to-unlock live frames and input without a device-list refresh, exact tray restart sequence, transfer-modal pause/resume/cancel/final-save/open behavior, and ordinary Viewer keyboard/mouse control. Android remains out of scope.
- 2026-09-18 transfer UI clarification: RED reproduced both redundant actions in the real browser (50 pass, 2 fail). GREEN passed 52/52 Viewer browser cases, 2/2 desktop-save cases and 2/2 native download-path cases; the Tauri library compiles for Windows x86. The tools now start a remote-file request only, lifecycle controls stay in the status modal, and the completed desktop row invokes the exact auto-saved file path after native download-root containment validation. Installed file opening remains a next-build physical check.
- 0.1.107 release-gate command correction: After the applicable predeploy gate passed with `ready-to-deploy`, the complete-stage command was also run and correctly rejected that status plus the still-pending deployment evidence. No commit, build, upload or policy write had started. Release preparation now stops on the applicable predeploy result; the complete-stage gate is reserved for the post-deployment `verified` contract.
- 0.1.107 live release and rollout: The normal main publisher succeeded, the exact public bytes and latest manifest were re-downloaded and verified, and all four Firebase aliases were checked. The rollout policy was changed from paused single-device 0.1.103 to active unrestricted General 100 percent for 0.1.107 using the prior document update time as an atomic precondition. Device-level pause data was not overwritten; AGENT-44CA7CAF remains paused. Stale device presence means this proves availability and eligibility, not completed installation or physical behavior.
...
- Status: Implemented on main and verified in GitHub Actions run 35177780697.
...
- Fix commit(s): `65972e9571c22f6e414f3e0838189cc14593f825`.
...
- Release proof: Main push 65972e9 triggered GitHub Actions run 35177780697. The executable regression comparison tests and existing development/recurrence gate passed on Ubuntu. Both build-release and publish-release were skipped; no product build, publication or device installation occurred.
- Remaining blocker: None for the requested local/CI wiring. Prose truth, regression classification and actual product behavior still require evidence review. Branch protection and equivalent gates in other repositories are not configured by this change.
