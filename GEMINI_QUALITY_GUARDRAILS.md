...
릴리스 빌드는 `main`의 `Prepare WonRemote v...` 커밋에서만 시작한다. 빌드 전에 태그를 수동 생성하지 않으며, 검증된 산출물 게시 단계가 버전 태그를 생성한다. 이 규칙은 릴리스 간 기본 브랜치 캐시를 재사용하고 실패한 빌드를 최신 태그로 노출하지 않기 위한 필수 조건이다.
