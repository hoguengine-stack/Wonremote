...
describe("mobile Viewer packaging", () => {
  it("installs the Viewer route without reusing the Windows update flow", () => {
    ...
    const viewerHeaders = Object.fromEntries(
      firebaseConfig.hosting.headers
        .find(({ source }: { source: string }) => source === "/viewer")
        .headers.map(({ key, value }: { key: string; value: string }) => [key, value]),
    );

    expect(manifest).toMatchObject({ id: "/viewer", start_url: "/viewer", display: "standalone" });
    expect(viewerHeaders["Cache-Control"]).toBe("no-cache, no-store, must-revalidate");
