...
const mapping = ["org.webrtc.ContextUtils", "org.webrtc.PeerConnectionFactory", "org.webrtc.PeerConnection", "org.webrtc.DataChannel", "org.webrtc.NetworkMonitor", "org.jni_zero.JniInit"]
  .map(name => `${name} -> ${name}:`)
  .join("\n") + "\n    1:2:void onNetworkThreadReady():20:21 -> onNetworkThreadReady\n";

describe("optimized Android JNI release boundary", () => {
  it("keeps the JNI zero initialization package required by the native library", () => {
    const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
    const rules = readFileSync(path.join(root, "mobile/android/agent/proguard-rules.pro"), "utf8");
    expect(rules).toContain("-keep class org.jni_zero.** { *; }");
  });
  ...
  it("blocks native class removal and renaming observed in the shipped mapping", () => {
    expect(() => verifyNativeMapping(mapping.replace("org.jni_zero.JniInit -> org.jni_zero.JniInit:\n", "")))
      .toThrow("missing");
