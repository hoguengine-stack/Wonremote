...
describe("installer update result", () => {
  it.each(["", "\uFEFF"])("restores a retained rollback result including Windows PowerShell BOM %j", async (bom) => {
    ...
    try {
      ...
      await writeFile(resultPath, bom + JSON.stringify({ state: "rollback", error: "new runtime failed", targetVersion: "0.1.62", updatedAt: "2026-08-16T00:00:00.000Z" }));
